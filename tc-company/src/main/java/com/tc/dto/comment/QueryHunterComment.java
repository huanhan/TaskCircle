...
public class QueryHunterComment extends QueryBasicComment{
    ...
    public static List<Predicate> initPredicates(QueryHunterComment queryHunterComment, Root<CommentHunter> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        predicates.add(QueryUtils.equals(root.get(CommentUser.COMMENT).get(Comment.ID),cb,queryHunterComment.getId()));
        predicates.add(QueryUtils.equals(root.get(CommentUser.COMMENT).get(Comment.TYPE),cb,queryHunterComment.getType()));
        predicates.add(QueryUtils.equals(root.get(CommentUser.COMMENT).get(Comment.CREATION_ID),cb,queryHunterComment.getCreationId()));
        predicates.add(QueryUtils.like(root.get(CommentUser.COMMENT).get(Comment.CONTEXT),cb,queryHunterComment.getContext()));

        predicates.add(QueryUtils.between(root.get(CommentUser.COMMENT).get(Comment.CREATE_TIME), cb, queryHunterComment.getCreateTimeBegin(), queryHunterComment.getCreateTimeEnd()));
        predicates.add(QueryUtils.between(root.get(CommentUser.COMMENT).get(Comment.NUMBET), cb, queryHunterComment.getNumberBegin(), queryHunterComment.getNumberEnd()));
